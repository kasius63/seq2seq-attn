...
local data2 = torch.class("data2")

function data2:__init(opt, data_file)
  ...
  local max_targ_l = self.target_l:max()
  ...
  for i = 1, self.length do
    ...
    if opt.use_chars_enc == 1 then
      ...
      source_i = self.source:sub(self.batch_idx[i], self.batch_idx[i]+self.batch_l[i]-1,
        1, max_source_l):transpose(1,2)
    end
    if opt.reverse_src == 1 then
      source_i = source_i:index(1, source_l_rev[{{max_source_l-max_source_l+1,
            max_source_l}}])
    end
...
function data2.__index(self, idx)
